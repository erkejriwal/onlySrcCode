...
public class LazyUpdationSegmentSumTree {
	...
	public LazyUpdationSegmentSumTree(int[] inputArray) {
	...
	private int findSumUtil(int queryStartIndex, int queryEndIndex,
		...
		if(this.lazy[stIndex]!= 0){
			int value = (originalArrayEndIndex - originalArrayStartIndex + 1)
					* this.lazy[stIndex];
			this.segmentArray[stIndex]+=value;
			if(originalArrayStartIndex != originalArrayEndIndex){
				this.lazy[this.getLeftChildIndex(stIndex)]+=this.lazy[stIndex];
				this.lazy[this.getRightChildIndex(stIndex)]+=this.lazy[stIndex];
			}
			this.lazy[stIndex]=0;
		}
...
//	}
	...
	private void updateValueRangeUtil(int updateDifference,
		...
		//updateDifference resolution
		if(updationRangeStartIndex<= originalArrayStartIndex &&
				originalArrayEndIndex <= updationRangeEndIndex){
			int value = (originalArrayEndIndex - originalArrayStartIndex + 1)
					* updateDifference;
			this.segmentArray[stIndex]+=value;
			if(originalArrayStartIndex != originalArrayEndIndex){
				this.lazy[this.getLeftChildIndex(stIndex)]+=updateDifference;
				this.lazy[this.getRightChildIndex(stIndex)]+=updateDifference;
			}
			return;
		}
		int mid = (originalArrayEndIndex + originalArrayStartIndex) / 2;
		...
		this.segmentArray[stIndex] = 
				this.segmentArray[this.getLeftChildIndex(stIndex)]
